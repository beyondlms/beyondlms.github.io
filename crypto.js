...
// ── Persistence ──
function loadCoins() { try { const s=localStorage.getItem('crypto_coins_v4'); if(s)return JSON.parse(s); } catch{} return DEFAULT_COINS.map(c=>({...c})); }
...
function wsConnectBinanceBatch(symbols, connKey) {
  ...
  try {
    ...
    ws.onopen = () => { wsRetryCount[connKey] = 0; updateWsStatus(); };
...
function showToast(msg, type = 'info') {
  ...
  el.innerHTML = `<div class="toast-title">${title}</div><div class="toast-body">${msg}</div>`;
...
function importData(event) {
  ...
  reader.onload = e => {
    try {
      ...
      if (data.coins) { coins = data.coins; saveCoins(); }
      if (data.alerts) {
