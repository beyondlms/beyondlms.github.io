// ── Config ──
let portfolio = [];
try { const s = localStorage.getItem('crypto_portfolio'); if (s) portfolio = JSON.parse(s); } catch {}
...
function debouncedSave(key, data, delay = 500) {
  clearTimeout(_pendingSaves[key]);
  _pendingSaves[key] = setTimeout(() => {
    localStorage.setItem(key, typeof data === 'string' ? data : JSON.stringify(data));
  }, delay);
}
function saveCoins() { debouncedSave('crypto_coins_v4', coins); }
function loadCoinApiMap() { try { const s=localStorage.getItem('crypto_coin_api'); if(s)return JSON.parse(s); } catch{} return {}; }
function saveCoinApiMap() { debouncedSave('crypto_coin_api', coinApiMap); }
function loadAlerts() { try { const s=localStorage.getItem('crypto_alerts'); if(s)return JSON.parse(s); } catch{} return []; }
function saveAlerts() { debouncedSave('crypto_alerts', alerts); }
...
function getSortedCoins() {
  ...
  const noData = [];
  coins.forEach(c => {
    if (priceHistory[c.symbol]) withData.push(c); else noData.push(c);
  });
  withData.sort((a, b) => {
    const va = getSortVal(a.symbol, k);
    const vb = getSortVal(b.symbol, k);
    return (va - vb) * dir;
  });
  return [...withData, ...noData]; // 无数据币种始终沉底
}
...
function testVoiceAlert() {
  ...
  // 临时启用语音进行测试（不写入 localStorage）
  const origVoiceEnabled = voiceEnabled;
  voiceEnabled = true;
  doSpeak(`语音测试，${testCoinName}当前价格八万八千美元，预警已触发！`);
  voiceEnabled = origVoiceEnabled; // 测试后恢复原设置
  if (isIOS) {
...
function checkAlerts(sym, currentPrice) {
  ...
  alerts.forEach(a => {
    ...
    if (hit) {
      ...
      // 价格回到正常范围
      if (a._everTriggered) {
        a._everTriggered = false; // 只在状态变化时重置
      }
    }
  });
...
const TRENDING_COOLDOWN = 60000; // 60秒内不重复拉取

async function fetchTrending() {
  const now = Date.now();
  if (_fetchingTrending) return;
  if (now - _lastTrendingFetch < TRENDING_COOLDOWN) {
    console.log(`[Trending] 跳过重复请求（距上次 ${(now - _lastTrendingFetch)/1000|0}s，冷却 ${TRENDING_COOLDOWN/1000}s）`);
    return;
  }
