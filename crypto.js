...
let wsBinanceLastDataTime = {}; // 单独追踪 Binance 最后数据时间（用于 OKX 回退判断）
    ...
    ws.onmessage = function(event) {
      ...
      try {
        ...
        if (data.data && data.data.s) {
          ...
          wsBinanceLastDataTime[sym] = Date.now();
        }
      } catch {}
      // 然后调用原始处理逻辑
    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        arr.forEach(d => {
          ...
          // 使用独立的 Binance 数据时间追踪，避免被 OKX 数据覆盖
          const lastBinanceTs = wsBinanceLastDataTime[sym] || 0;
          const binanceStale = (Date.now() - lastBinanceTs) > 15000; // Binance data > 15s old
...
// 事件监听器注册标志（防止重复注册导致内存泄漏）
let _audioListenersRegistered = false;
...
function recoverAudioContext() {
  try {
    if (!audioCtx || audioCtx.state === 'closed') {
      ...
      audioUnlocked = false;
      // 只注册一次监听器（防止内存泄漏）
      if (!_audioListenersRegistered) {
        document.addEventListener('touchstart', unlockAudio, { once: false, passive: true });
        document.addEventListener('touchend', unlockAudio, { once: false, passive: true });
        document.addEventListener('click', unlockAudio, { once: false });
        _audioListenersRegistered = true;
      }
      return;
    }
    if (audioCtx.state === 'suspended') {
...
// ── Toast ──
const MAX_TOASTS = 5; // 最大 Toast 通知数量

function showToast(msg, type = 'info') {
  const container = document.getElementById('toastContainer');

  // 限制 Toast 数量，防止 DOM 膨胀
...
window.addEventListener('beforeunload', () => {
  // 清理主要定时器
  if (timer) clearInterval(timer);
  if (forceMonitorTimer) clearInterval(forceMonitorTimer);
  if (_audioHealthTimer) clearInterval(_audioHealthTimer);

  // 清理 WebSocket 健康检查定时器
  if (KEEPALIVE?.wsHealthTimer) clearInterval(KEEPALIVE.wsHealthTimer);
  if (KEEPALIVE?.driftTimer) clearInterval(KEEPALIVE.driftTimer);
  ...
  console.log('[Cleanup] 页面卸载，定时器和连接已清理');
});
